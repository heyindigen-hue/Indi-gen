...
export function PushEditor({ template, open, onClose }: PushEditorProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      api.post(`/admin/templates/push/${template!.id}`, { title, body }),
    onSuccess: () => {
      toast.success('Push template saved');
      queryClient.invalidateQueries({ queryKey: ['admin-templates-push'] });
    },
    ...
    <Sheet open={open} onOpenChange={(isOpen) => { if (!isOpen) onClose(); }}>
      <SheetContent className="w-[480px] sm:max-w-[480px] flex flex-col gap-0 overflow-y-auto">
        <SheetHeader className="pb-4">
          <SheetTitle>{template?.name ?? 'Push Template'}</SheetTitle>
        </SheetHeader>
        ...
        <SheetFooter className="pt-4 flex-row gap-2 justify-end">
          ...
          <Button
            ...
            disabled={saveMutation.isPending || title.length > TITLE_MAX || body.length > BODY_MAX}
          >
