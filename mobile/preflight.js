 ...
 *
 * Phase-aware: split between EAS pre-install (file-only) and post-install
 * (deps required) so we don't get spurious failures before npm install.
 *
 * Phases:
 *   --phase=pre   -> EAS eas-build-pre-install   (file-only checks)
 *   --phase=post  -> EAS eas-build-post-install  (node_modules-required checks)
 *   --phase=local -> default; runs everything
 *
...
const PHASE = (process.argv.find(a => a.startsWith('--phase=')) || '--phase=local').slice(8);
const RUN_PRE  = PHASE === 'pre'  || PHASE === 'local';
const RUN_POST = PHASE === 'post' || PHASE === 'local';
...
// File-only setup — JSON reads work in any phase, no node_modules required.
const pkg = JSON.parse(fs.readFileSync(path.join(ROOT, 'package.json'), 'utf8'));
const appJson = JSON.parse(fs.readFileSync(path.join(ROOT, 'app.json'), 'utf8'));
const expoBlock = appJson.expo || {};
const plugins = expoBlock.plugins || [];

// ---------------------------------------------------------------------------
// 1. expo-doctor (POST: needs node_modules)
// ---------------------------------------------------------------------------
if (RUN_POST) {
...
// ---------------------------------------------------------------------------
// 2. tsc --noEmit (POST: needs typescript installed)
// ---------------------------------------------------------------------------
if (RUN_POST) {
...
// ---------------------------------------------------------------------------
// 3. require()/import asset paths (PRE: file-only)
// ---------------------------------------------------------------------------
if (RUN_PRE) {
...
// ---------------------------------------------------------------------------
// 4. node_modules sanity — every declared dep is installed (POST)
// ---------------------------------------------------------------------------
if (RUN_POST) {
...
// ---------------------------------------------------------------------------
// 5. app.json plugins are installed packages (POST: checks node_modules dirs)
// ---------------------------------------------------------------------------
if (RUN_POST) {
...
// ---------------------------------------------------------------------------
// 6. babel.config.js — no deprecated SDK 50 plugins, reanimated last (PRE)
// ---------------------------------------------------------------------------
if (RUN_PRE) {
...
// ---------------------------------------------------------------------------
// 7. eas.json profiles (PRE: file-only)
// ---------------------------------------------------------------------------
if (RUN_PRE) {
...
// ---------------------------------------------------------------------------
// 8. app.json schema — invalid SDK 50 keys (PRE: file-only)
// ---------------------------------------------------------------------------
if (RUN_PRE) {
  ...
  ok('app.json schema check complete');
}

// ---------------------------------------------------------------------------
// 9. assets referenced from app.json exist on disk (PRE: file-only)
// ---------------------------------------------------------------------------
if (RUN_PRE) {
...
// ---------------------------------------------------------------------------
// 10. metro bundle simulation — only on Linux/Mac (POST: needs expo + metro)
// ---------------------------------------------------------------------------
if (RUN_POST) {
...
// ---------------------------------------------------------------------------
// 11. expo install --check — native module versions (POST: needs expo)
// ---------------------------------------------------------------------------
if (RUN_POST) {
...
// ---------------------------------------------------------------------------
// 12. main entry resolves (POST: resolves into node_modules)
// ---------------------------------------------------------------------------
if (RUN_POST) {
...
// ---------------------------------------------------------------------------
// 13. .npmrc legacy-peer-deps for EAS (PRE: file-only)
// ---------------------------------------------------------------------------
if (RUN_PRE) {
  step(13, TOTAL, '.npmrc');
...
// ---------------------------------------------------------------------------
// 14. babel transform smoke test (POST: needs @babel/core + expo-router)
// ---------------------------------------------------------------------------
if (RUN_POST) {
...
// ---------------------------------------------------------------------------
console.log('\n=== PREFLIGHT SUMMARY ===');
console.log(`Phase: ${PHASE}  (RUN_PRE=${RUN_PRE}, RUN_POST=${RUN_POST})`);
